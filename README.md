...
- **输入框工具栏**：可选地在聊天输入区顶部显示与“按钮与动作”共享配置的单层 Liquid Glass 快捷工具栏；每个 `MMInputToolView` 拥有自己的工具栏实例，横向边界对齐输入行左右两个圆形按钮，使用紧凑 Liquid Glass 按钮和系统按压反馈，随输入框位置、多行输入和输入层级内的引用内容自动过渡。工具栏不显示语音转述或斗图助手动作；这两个动作继续由环形菜单使用原有入口。工具栏宽度按按钮数量自适应，按钮不足时居中，按钮过多时保留横向滚动。工具栏占用高度会同步到当前聊天表格的底部 inset，避免消息被遮挡；布局只在输入视图自身发生变化时更新，不参与聊天列表滚动，动作仍由现有路由执行。
