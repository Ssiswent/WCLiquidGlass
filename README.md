...
插件以 dylib 形式注入微信进程：`%ctor` 完成初始化，`WCLiquidGlassManager` 单例在微信之上叠加一层不抢焦点的透明 `UIWindow` 来承载环形菜单，所有按钮动作都通过 Objective-C 运行时反射调用微信自身的方法，插件本身不实现任何微信业务逻辑。
...
覆盖层窗口的 `windowLevel` 高于系统弹窗，`canBecomeKeyWindow` 返回 `NO`，`hitTest:` 命中窗口自身时返回 `nil`，因此除按钮区域外的触摸全部透传给微信，键盘焦点也不会被抢走。菜单内容与布局来自本地配置、微信页面 hook 回调，以及键盘和输入区的实时状态。
...
flowchart TD
  ...
  MGR -. "reload" .-> HV
  WXHOOK["微信页面 hooks：聊天页进出与输入事件"] -. "刷新菜单和动作状态" .-> MGR
  KB["键盘通知 + 输入区状态"] -. "避让与布局更新" .-> HV
