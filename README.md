...
“液态效果”统一控制悬浮入口、环形菜单、聊天时间条、聊天底部菜单和消息长按菜单的材质取向：
...
- **聊天底部菜单液态**：在微信原生附件菜单完成布局后，仅替换其背景层；照片、拍摄、文件等按钮的内容、布局、点击与关闭行为仍由微信原生菜单负责，不依赖素材仓插件。
- **消息长按菜单液态**：开启后仅接管 WCGlass 消息长按菜单的呈现层；按钮文字、图标、数量、点击与长按业务逻辑仍由微信原生菜单负责。展开和收起使用连续圆角玻璃与弹簧动画，并跟随“清透 / 均衡 / 着色”。
...
- 聊天时间条液态、聊天底部菜单液态、消息长按菜单液态、首页圆角与液态卡片。
- 16 个动作以内的按钮管理与顺序调整。
...
| --- | --- |
| 消息长按菜单液态 | 可由“消息长按菜单液态”开关接管 WCGlass 菜单的视觉呈现，保留微信原生菜单内容与交互。 |
