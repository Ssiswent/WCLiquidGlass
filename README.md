...
恢复默认和配置操作使用锚定到实际触发控件的原生 Action Sheet；日志 Page Sheet 的“清空”按钮使用
无背景的系统 `UIBarButtonItem(menu:)`，菜单只提供带垃圾桶图标的“确认清空”动作，不叠加第二个弹窗。
...
| 3.0.4-6 类名混淆 | 设置入口与底栏搜索模式同时兼容旧类名及已确认的新类名；WCGlass 尚未加载时延后安装，不枚举运行时类或调用其混淆内部入口。 |
