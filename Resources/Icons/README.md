...
- `Source`：可编辑母版。品牌只允许使用已确认的 `Brand.png`；其余图标使用 SVG。`Source/dark` 保存由对应品牌 PNG 或 SVG 换色得到的相同几何深色模式母版，使用暖白主层与低对比灰色后层。
...
深色模式图标更新使用 `zsh Resources/Icons/render-settings-icons.zsh`；脚本会从已确认的 `Source/Brand.png` 生成 `Source/dark/Brand.png`。如同时修改浅色母版，使用 `zsh Resources/Icons/render-settings-icons.zsh all`。
