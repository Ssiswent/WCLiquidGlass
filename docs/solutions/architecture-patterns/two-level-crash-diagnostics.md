...
- 支持单条删除；完整日志页面复用按钮与动作页的背景和导航上下文，右上角始终使用系统 plain `UIBarButtonItem` 和默认 `UIMenu`，有日志时显示带垃圾桶图标的“确认清空”，无日志时显示信息图标和“暂无日志”，点击外部取消，不叠加 Alert 或 Action Sheet。按钮颜色和原生元素不随日志数量变化。删除前校验目标位于 Crashes 目录。
- 超过 20 份时按修改时间删除最旧文件。
