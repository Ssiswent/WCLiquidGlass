...
- 支持单条删除；日志 Page Sheet 右上角使用系统 `UIBarButtonItem(menu:)`，菜单只显示带垃圾桶图标的“确认清空”，点击外部取消，不叠加 Alert 或 Action Sheet。按钮颜色和可点击状态不随日志数量变化；无日志时只移除菜单，点击不执行操作。删除前校验目标位于 Crashes 目录。
- 超过 20 份时按修改时间删除最旧文件。
