...
WCLiquidGlass 是一个注入微信进程的 rootless arm64 Theos tweak。它向微信插件列表注册设置入口，并在微信各页面之上提供一个可拖动、自动吸边、闲置半隐藏的快捷菜单。菜单会根据当前页面能力过滤不可执行动作，设置页则保留完整配置，避免用户配置随页面切换丢失。
...
10. **环形菜单保持纯图标。** 不显示文字和角标；可选液态面板使用图标加最多两行标题。收起锚点使用独立、固定图标，不复用任何功能按钮图标。
