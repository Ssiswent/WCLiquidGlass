...
WCLiquidGlass 1.9.1 将这项可验证的结构原则用于可选“液态面板”：独立圆形锚点与圆角面板作为同一 `UIGlassContainerEffect` 的 sibling 真实玻璃表面，面板从靠近锚点的种子 frame 过渡到图标加标题的动作网格。它仍自行维护动作过滤、路由和覆盖窗口，不访问 WCGlass，也不宣称使用了其未公开的阻尼或私有实现。
