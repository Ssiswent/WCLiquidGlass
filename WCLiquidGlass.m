...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return WCLiquidGlassFooterLabel(@"在“按钮与动作”页面点按编辑，即可添加、删除或拖动调整按钮顺序。聊天时间条、通讯录 A–Z 索引与长按菜单跟随本插件材质设置；未读消息提示和首页圆角与液态可分别管理各自的圆角、间距与材质。");
    }
    if (section == 2) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    ...
    if (section == 1) {
        return WCLiquidGlassFooterHeight(@"在“按钮与动作”页面点按编辑，即可添加、删除或拖动调整按钮顺序。聊天时间条、通讯录 A–Z 索引与长按菜单跟随本插件材质设置；未读消息提示和首页圆角与液态可分别管理各自的圆角、间距与材质。", 104.0);
    }
    if (section == 2) {
